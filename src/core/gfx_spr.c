...
static void spr_blit_display(u32 *restrict dp,
                             u32 sp, u32 sm, int mode)
{
    switch (mode) {
    case SPR_MODE_INV: sp = ~sp; // fallthrough
...
static void spr_blit_row_rev_display(spr_blit_s info)
{
    u32 pt = info.pat.p[info.y & 7];
    if (pt == 0) return;
    u32 *restrict dp       = (u32 *restrict)info.dp;
    ...
    if (a == 0) { // same alignment, fast path
        ...
        u32 m = sm ? brev32(*sm--) & info.ml : info.ml;
        for (int i = 0; i < info.dmax; i++) {
            spr_blit_display(dp, p, m & pt, info.mode);
            p  = brev32(*sp--);
            m  = sm ? brev32(*sm--) : 0xFFFFFFFFU;
            dp = dp + 1;
        }
        spr_blit_display(dp, p, m & (pt & info.mr), info.mode);
        return;
    }
...
static void spr_blit_row_display(spr_blit_s info)
{
    u32 pt = info.pat.p[info.y & 7];
    if (pt == 0) return;
    u32 *restrict dp       = (u32 *restrict)info.dp;
    ...
    for (int i = 1; i < info.dmax; i++) {
        u32 p = bswap32((bswap32(*sp) << a) | (bswap32(*(sp + 1)) >> b));
        u32 m = sm ? bswap32((bswap32(*sm) << a) | (bswap32(*(sm + 1)) >> b)) & pt : pt;
        spr_blit_display(dp, p, m, info.mode);
        sp = sp + 1;
        sm = sm ? sm + 1 : sm;
        dp = dp + 1;
    }
...
void gfx_spr_display(gfx_ctx_s ctx, texrec_s src, v2_i32 pos, int flip, int mode)
{
    ...
    for (info.y = y1; info.y <= y2; info.y++) {
        if (flip & SPR_FLIP_X) {
            spr_blit_row_rev_display(info);
        } else {
            spr_blit_row_display(info);
        }
