...
static i32 cos_q16_fast(i32 turn_q10)
{
    int i   = (uint)turn_q10 & 0x3FF;
    int neg = 0;
    switch (i & 0x300) {                       // [0, 65535]
    case 0x100: i = 0x200 - i, neg = 1; break; // [65536, 131071]
    case 0x200: i = i - 0x200, neg = 1; break; // [131072, 196607]
    case 0x300: i = 0x400 - i; break;          // [196608, 262143]
    }
    if (i == 0x100) return 0;
    i32 r = cos_table[i];
    return neg ? -r : r;
...
// p: angle, where 262144 = 0x400 = 2 PI
// output: [-65536, 65536] = [-1; +1]
static inline i32 sin_q16_fast(i32 turn_q10)
{
    return cos_q16_fast(turn_q10 - 0x100);
}
...
#if 1
    i32 a = cos_table[i >> 8];
    i32 b = 0x0FF00 < i ? 0 : cos_table[(i + 0xFF) >> 8];
    i32 r = a - (((a - b) * (i & 255)) >> 8);
#else
    i     = (i * i + 0x8000) >> 16;
...
static f32 sin_f_fast(f32 x)
{
    f32 i = fmodf(x, PI2_FLOAT) / PI2_FLOAT;
    return ((f32)sin_q16_fast((i32)(i * (f32)0x400)) * .000015258789f);
}

static f32 cos_f_fast(f32 x)
{
    f32 i = fmodf(x, PI2_FLOAT) / PI2_FLOAT;
    return ((f32)cos_q16_fast((i32)(i * (f32)0x400)) * .000015258789f);
}
