...
void biter_constrain(obj_s *o)
{
    ...
    for (i32 k = 0; k < 2; k++) {
        for (i32 n = 1; n < BITER_NUM_SEGS; n++) {
            ...
            v2_i32       p1  = b1->p;
            v2_i32       p2  = b2->p;
            v2_i32       dt  = v2_i32_sub(p1, p2);
            i32          lsq = v2_i32_lensq(dt);
            if (lsq <= BITER_SEG_DISTSQ) continue;

            i32    l     = sqrt_i32(lsq);
            i32    l_set = BITER_SEG_DIST + (l - BITER_SEG_DIST) / 2;
            v2_i32 dt2   = v2_i32_setlenl(dt, l, l_set);
            b1->p        = v2_i32_add(p2, dt2);
            b2->p        = v2_i32_sub(p1, dt2);
        }
        b->segs[0].p                  = p_head;
        b->segs[BITER_NUM_SEGS - 1].p = b->anchor;
    }
...
void biter_on_update(g_s *g, obj_s *o)
{
    ...
    case BITER_ST_IDLE: {
        ...
        if (100 <= o->timer) {
            ...
            v2_i32 tp = {(rp * sin_q16(ap)) >> 16,
                         (rp * cos_q16(ap)) >> 16};
            b->p_old  = b->p_new;
            b->p_new  = v2_i32_add(tp, b->anchor);
        }
    ...
    b->p         = p;
    v2_i32 p_end = v2_i32_add(p, b->p_rng);
    v2_i32 dt    = v2_i32_sub(p_end, b->anchor);
    dt           = v2_truncate(dt, BITER_MAX_R);
    p_end        = v2_i32_add(b->anchor, dt);
...
void biter_on_draw(g_s *g, obj_s *o, v2_i32 cam)
{
    ...
    for (i32 n = 0; n < BITER_NUM_SEGS; n++) {
        ...
        v2_i32       p_seg = v2_i32_add(bs->p, cam);
