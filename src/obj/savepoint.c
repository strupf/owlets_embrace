...
void savepoint_on_update(g_s *g, obj_s *o)
{
    ...
    for (i32 n = 0; n < NUM_SAVEPOINT_BUTTERFLY; n++) {
        ...
        for (i32 i = 0; i < NUM_SAVEPOINT_BUTTERFLY; i++) {
            ...
            v2_i32                 v_dt = v2_i32_sub(f->p_q8, t->p_q8);
            i32                    dst  = v2_i32_len(v_dt);
            if (dst && dst < 2000) {
                v_dt    = v2_i32_setlen(v_dt, (2000 - dst) / 20);
                f->v_q8 = v2_i32_add(f->v_q8, v_dt);
            }
        }

#define SAVEPOINT_CENTER_R 64

        i32 dsq = v2_i32_lensq(f->p_q8) >> 16; // distance squared in pixels
        if (SAVEPOINT_CENTER_R <= dsq) {       // keep around the savepoint
            v2_i32 d = v2_i32_mulq(f->p_q8, dsq, 20);
            f->v_q8  = v2_i32_sub(f->v_q8, d);
        } else { // but keep them from touching the center directly
            v2_i32 d = v2_i32_mulq(f->p_q8, SAVEPOINT_CENTER_R - dsq, 1);
            f->v_q8  = v2_i32_add(f->v_q8, d);
        }
    ...
    for (i32 n = 0; n < NUM_SAVEPOINT_BUTTERFLY; n++) {
        s->f[n].p_q8 = v2_i32_add(s->f[n].p_q8, s->f[n].v_q8);
    }
}
...
void savepoint_on_draw(g_s *g, obj_s *o, v2_i32 cam)
{
    ...
    for (i32 n = 0; n < NUM_SAVEPOINT_BUTTERFLY; n++) {
        savepoint_butterfly_s *f = &s->f[n];
        v2_i32                 p = v2_i32_add(v2_i32_shr(f->p_q8, 8), p0);
