...
static void rope_points_in_tris(game_s *g, tri_i32 t1, tri_i32 t2, v2_arr *pts)
{
        ...
        for (int y = y1; y <= y2; y++) {
                for (int x = x1; x <= x2; x++) {
                        ...
                        for (int i = 0; i < 3; i++) {
                                try_add_point_in_tri(ttri1.p[i], t1, t2, pts);
                                try_add_point_in_tri(ttri2.p[i], t1, t2, pts);
                        }
                }
        }
        ...
        for (int n = 0; n < solids.n; n++) {
                ...
                const v2_i32 points[4] = {o->pos.x, o->pos.y,
                                          o->pos.x + o->w, o->pos.y,
                                          o->pos.x, o->pos.y + o->h,
...
void rope_moved_by_solid(game_s *g, rope_s *r, obj_s *solid, v2_i32 dt)
{
        ...
        for (int n = 0; n < solids.n; n++) {
                const obj_s *o          = solids.o[n];
                const v2_i32 opoints[4] = {o->pos.x, o->pos.y,
                                           o->pos.x + o->w, o->pos.y,
                                           o->pos.x, o->pos.y + o->h,
                                           o->pos.x + o->w, o->pos.y + o->h};
                for (int i = 0; i < 4; i++) {
                        if (!v2_arrcontains(points, opoints[i]))
                                v2_arradd(points, opoints[i]);
                }
        }
                         ...
                         ropenode_s *rp, ropenode_s *rc, ropenode_s *rn)
{
        ...
        for (int n = 0; n < solids.n; n++) {
                obj_s *o          = solids.o[n];
                v2_i32 opoints[4] = {o->pos.x, o->pos.y,
                                     o->pos.x + o->w, o->pos.y,
                                     o->pos.x + o->w, o->pos.y + o->h,
                                     o->pos.x, o->pos.y + o->h};
                for (int i = 0; i < 4; i++) {
             ...
             r1 = r2, r2 = r2->next) {
                ...
                for (int y = y1; y <= y2; y++) {
                        for (int x = x1; x <= x2; x++) {
                                ...
                                if (t == 0) continue;
                                tilecollider_s tc  = tilecolliders[t - 1];
                                tri_i32        tr1 = translate_tri_xy(tc.tris[0], x * 16, y * 16);
                                tri_i32        tr2 = translate_tri_xy(tc.tris[1], x * 16, y * 16);

                                bool32 a = overlap_tri_lineseg_excl(tr1, ls);
                                bool32 b = overlap_tri_lineseg_excl(tr2, ls);
                                if (a || b) {
                                        PRINTF("TILES\n");
                                        overlap_tri_lineseg_excl(tr1, ls); // ony for stepping through
                ...
                for (int n = 0; n < solids.n; n++) {
                        ...
                        if (a || b) {
                                PRINTF("SOLID\n");
                                overlap_tri_lineseg_excl(tris[0], ls); // ony for stepping through
