...
const hero_jumpvar_s g_herovar[NUM_HERO_JUMP] = {
    ...
    {Q_VOBJ(4.2), 30, Q_VOBJ(0.29), Q_VOBJ(0.13)},  // ground
...
void   hero_on_update_do(g_s *g, obj_s *o, inp_s inp);
...
i32 hero_special_input(g_s *g, obj_s *o, inp_s inp)
{
    ...
    if (h->unhooked_mom) {
        h->unhooked_mom--;
        o->v_q12.y -= lerp_i32(Q_VOBJ(0.0), Q_VOBJ(0.20), h->unhooked_mom, HERO_TICKS_UNHOOKED_MOM);
    }
...
#if PLTF_SDL
    if (pltf_sdl_key(SDL_SCANCODE_Q)) {
        crankdta_q16 = 1200;
    }
#endif

#if 0
    if (50 <= crankdta_q16 && !h->crank_cooldown) {
        h->crank_swap_buildup += crankdta_q16;

        if (HERO_CRANK_BUILDUP_Q16 <= h->crank_swap_buildup) {
            swap_mode             = 1;
        }
    } else {
        h->crank_swap_buildup = (h->crank_swap_buildup * 240) >> 8;
    }
#endif

    if (SETTINGS.swap_ticks <= h->b_hold_tick) {
        swap_mode = 1;
    }

    if (swap_mode) {
        h->b_hold_tick        = 0;
        h->crank_swap_buildup = 0;
        h->mode               = 1 - h->mode;
        ...
        companion_on_enter_mode(g, ocomp, h->mode);
    }

    bool32 may_swap = hero_has_upgrade(g, HERO_UPGRADE_HOOK) &&
                      hero_has_upgrade(g, HERO_UPGRADE_COMPANION);
    bool32 can_attack = h->mode == HERO_MODE_COMBAT &&
                        !h->crouched &&
                        !h->walljump_tick &&
                        !h->stomp;
    bool32 can_use_hook = hero_has_upgrade(g, HERO_UPGRADE_HOOK) &&
                          h->mode == HERO_MODE_NORMAL &&
                          !h->crouched &&
                          !h->walljump_tick &&
                          !h->stomp;
    ...
    if (can_use_hook) {
        ...
        if (APP.opt) {
            ...
            if (!inps_btn(inp, INP_B)) {
                if (!mm && o->rope) {
                    grapplinghook_anchor_in_place(g, &g->ghook);
                    mm = 1;
                } else {
                }
                ...
                if (o->rope) {
                    ...
                    if (dpad_x | dpad_y) {
                        ...
                        hero_action_throw_grapple(g, o, ang, 3000);
                    } else if (may_swap) {
                        h->b_hold_tick = 1;
                    }
                }
...
#define HERO_SAFE_NUM_TILES 3
    ...
    for (i32 x = x1; x <= x2; x++) {
        if (g->tiles[x + y * g->tiles_x].shape != TILE_BLOCK)
            return;
    }
...
void hero_on_update(g_s *g, obj_s *o, inp_s inp)
{
    ...
    bool32 grounded = obj_grounded(g, o);
    if (grounded && !h->was_grounded_upd) {
        for (obj_each(g, i)) {
            ...
            if (overlap_rec(obj_rec_bottom(o), rtop)) {
                i->on_impulse(g, i, o->v_q12.x, o->v_q12.y);
            }
        }
    }
    ...
    o->ropeobj.v_q8.x = o->v_q12.x >> 4;
    o->ropeobj.v_q8.y = o->v_q12.y >> 4;
    o->ropeobj.a_q8.x = 0;
    o->ropeobj.a_q8.y = HERO_GRAVITY << 4;
    o->ropeobj.m_q8   = 256;
    hero_check_safe_pos(g, o);
    bool32 special_input = hero_special_input(g, o, inp);
    if (special_input) {
        hero_on_update_do(g, o, inp);
    }
    h->was_grounded_upd = grounded;
}
...
void hero_on_update_do(g_s *g, obj_s *o, inp_s inp)
{
    ...
    if (st == HERO_ST_WATER && h->state_prev != HERO_ST_WATER) {
        ...
        for (i32 n = 0; n < g->n_fluid_areas; n++) {
            ...
            if (overlap_rec(obj_aabb(o), rfa)) {
                fluid_area_impact(fa, splashpos.x - fa->x, 12, o->v_q12.y >> 2,
                                  FLUID_AREA_IMPACT_FLAT);
            }
        }

        i32 particleID = PARTICLE_EMIT_ID_HERO_WATER_SPLASH;
        if (Q_VOBJ(5.86) <= o->v_q12.y) {
    ...
    bool32 might_jump = !h->attack_tick;
    if (h->ticks_health) {
        h->ticks_health--;
    }
    if (h->walljump_tick) {
    ...
    if (h->attack_tick) {
        ...
        case HERO_ATTACK_GROUND: {
            ...
            ticks_total = ani_len(ANIID_HERO_ATTACK);
            break;
        }
        }
    ...
    if ((o->bumpflags & OBJ_BUMP_Y) && !(o->bumpflags & OBJ_BUMP_Y_BOUNDS)) {
        ...
        if (st == HERO_ST_AIR && rope_stretched) {
            ...
            if (Q_VOBJ(4.0) <= o->v_q12.y) {
                f32 vol = (0.5f * (f32)o->v_q12.y) / Q_VOBJ(4.0);
            }
            if (Q_VOBJ(2.0) <= o->v_q12.y) {
                v2_i32 posc = obj_pos_bottom_center(o);
                posc.y -= 2;
                i32 pID = (Q_VOBJ(4.0) <= o->v_q12.y ? PARTICLE_EMIT_ID_HERO_LAND_HARD
                                                     : PARTICLE_EMIT_ID_HERO_LAND);
                particle_emit_ID(g, pID, posc);
            }
            if (Q_VOBJ(1.5) <= o->v_q12.y) {
                h->impact_ticks = min_i32(o->v_q12.y >> 12, 8) + 2;
                snd_play(SNDID_LANDING,
                         (0.35f * min_f32((f32)o->v_q12.y, Q_VOBJ(3.0))) / (f32)Q_VOBJ(3.0),
                         rngr_f32(0.9f, 1.1f));
            }
    ...
    if (o->bumpflags & OBJ_BUMP_X) {
        h->sprint = 0;
        if (st == HERO_ST_AIR && rope_stretched) {
            if (dpad_x == sgn_i32(o->v_q12.x) || abs_i32(o->v_q12.x) < Q_VOBJ(2.34)) {
                o->v_q12.x = 0;
            } else {
                obj_vx_q8_mul(o, -64);
            }
    ...
    case HERO_ST_STOMP: {
        ...
        if (r && (r->len_max_q4 + (0)) <= (rope_len_q4(g, r))) {
            h->stomp = 0;
#if 1
            ...
            if (dpad_x) {
                ...
                ptang    = v2_i32_setlen(ptang, Q_VOBJ(0.10));
                o->v_q12 = v2_i32_add(o->v_q12, ptang);
            }
#endif
#else
            ropenode_s *rn1  = ropenode_neighbour(r, o->ropenode);
            v2_i32      dtrn = v2_i32_sub(rn1->p, o->ropenode->p);
            dtrn.x >>= 1;
            dtrn = v2_i32_setlen(dtrn, 3000);
            grapplinghook_destroy(g, &g->ghook);
            o->v_q12.x = dtrn.x;
            o->v_q12.y = dtrn.y;
#endif
...
#define LOW_GRAV_PK +0            // v q8 with lowest gravity
        ...
        if (o->v_q12.y >= Q_VOBJ(7.0)) {
            o->animation++;
            g->cam.chero.force_higher_floor = min_i32(o->animation >> 1, 24);
        } else {
            o->animation = 0;
        }
        ...
        // o->v_q12.y = min_i32(o->v_q12.y, Q_VOBJ(7.0));
        obj_move_by_v_q12(g, o);
        break;
    }
    }
    ...
    if (h->crouched < 0) {
        o->animation += (o->v_q12.x * o->facing) >> 4;
    } else if (st == HERO_ST_GROUND) {
        i32 va = abs_i32(o->v_q12.x) >> 4;
...
void hero_start_jump(g_s *g, obj_s *o, i32 ID)
{
    ...
    if (ID == HERO_JUMP_GROUND) {
        ...
        } else if (HERO_VX_SPRINT <= abs_i32(o->v_q12.x)) {
...
bool32 hero_action_ungrapple(g_s *g, obj_s *o)
{
    ...
    if (abs_i32(o->v_q12.x) < HERO_VX_WALK) {
        mulx = 285;
    }
...
void hero_do_inair(g_s *g, obj_s *o, inp_s inp)
{
    ...
    bool32 rope_stretched = (o->rope && (o->rope->len_max_q4 * 252) <= (rope_len_q4(g, o->rope) << 8));
    ...
    if (rope_stretched) { // swinging
        ...
        if (sgn_i32(o->v_q12.x) != dpad_x) {
            obj_vx_q8_mul(o, 254);
        }
        o->v_q12.x += ((dtrope_s == dpad_x) ? Q_VOBJ(0.20) : Q_VOBJ(0.05)) * dpad_x;
#else
        // apply swinging force tangentially to the swinging arc
        v2_i32 ptang = {-dtrope.y, +dtrope.x}; // tangent counter clockwise
        ...
        bool32 start_climbing = hero_has_upgrade(g, HERO_UPGRADE_CLIMB) &&
                                ...
                                !o->rope &&
                                (h->walljump_tick < WALLJUMP_TICKS - 2);
        ...
        if (start_climbing) {
            ...
            if (inps_btn_jp(inp, INP_A)) {
                ...
                h->b_hold_tick = 0;
                if (5 <= h->momentum) {
                    h->momentum_wall = ease_in_quad(5, 20, min_i32(h->momentum, 40), 40);
                } else {
                    h->momentum_wall = 0;
                }
            ...
            if (inps_btn(inp, INP_A)) {
                ...
                i32            ti = (pow_i32(h->jumpticks, 4) >> 8) - t0;
        ...
        if (do_x_movement) {
            ...
            if (vs == 0) {
                ...
                ax = lerp_i32(Q_VOBJ(0.78), Q_VOBJ(0.08), va, HERO_VX_WALK);
...
void hero_inair_jump(g_s *g, obj_s *o, inp_s inp, b32 can_air_jump)
{
    ...
    bool32 jump_wall = 0;
    if (can_air_jump) {
        for (i32 n = 0; n < HERO_WALLJUMP_THRESHOLD_PX; n++) {
...
void hero_do_swimming(g_s *g, obj_s *o, inp_s inp)
{
    ...
    if (h->swimming == SWIMMING_DIVING && hero_has_upgrade(g, HERO_UPGRADE_DIVE)) {
        obj_vy_q8_mul(o, 230);
        if (dpad_y) {
            i32 i0 = (dpad_y == sgn_i32(o->v_q12.y) ? abs_i32(o->v_q12.y) : 0);
            i32 ay = (max_i32(Q_VOBJ(2.0) - i0, 0) * 128) >> 8;
            o->v_q12.y += ay * dpad_y;
        }
    } else {
        o->v_q12.y += HERO_GRAVITY;
        obj_vy_q8_mul(o, Q_8(0.86));

        bool32 can_swim = hero_has_upgrade(g, HERO_UPGRADE_SWIM);
        i32    ch       = lerp_i32(HERO_GRAVITY,
                                   HERO_GRAVITY + Q_VOBJ(0.8),
        ...
        if (HERO_WATER_THRESHOLD <= water_depth &&
            ...
            o->v_q12.y = max_i32(o->v_q12.y, -vmin);
        }
    }
    ...
    for (i32 n = 0; n < g->n_fluid_areas; n++) {
        ...
        i32 ax = abs_i32(o->v_q12.x >> 4);
        hcenter.x += sgn_i32(o->v_q12.x) * 10;
        fluid_area_impact(fa, hcenter.x - fa->x, 10,
                          -ax / 15 + rngr_sym_i32(10) + ((8 * sin_q15(g->tick_animation << 13)) >> 15),
                          FLUID_AREA_IMPACT_FLAT);
    }

    if (dpad_x != sgn_i32(o->v_q12.x)) {
        o->v_q12.x /= 2;
    }
    if (dpad_x) {
        i32 i0 = (dpad_x == sgn_i32(o->v_q12.x) ? abs_i32(o->v_q12.x) : 0);
        i32 ax = (max_i32(Q_VOBJ(3.0) - i0, 0) * 32) >> 8;
        o->v_q12.x += ax * dpad_x;
    }
}
...
void hero_do_climbing(g_s *g, obj_s *o, inp_s inp)
{
    ...
    if (h->momentum_wall) {
        dpad_y           = -1;
        tomove           = clamp_i32(h->momentum_wall >> 2, 1, 3);
        h->momentum_wall = max_i32(h->momentum_wall - 1, 0);
    } else {
        sta = 0;
    }
    ...
    if (inps_btn_jp(inp, INP_A)) {
        ...
        if (!n_moved) {
            bool32 stop_climbing = 0;

            if (dpad_y < 0) {
                o->v_q12.y = -Q_VOBJ(4.3);
                // stop_climbing = 1;
        ...
        o->v_q12.y += Q_VOBJ(0.08);
        o->v_q12.y = min_i32(o->v_q12.y, Q_VOBJ(2.0));
        obj_move_by_q12(g, o, 0, o->v_q12.y);
    }
...
void hero_do_crawling(g_s *g, obj_s *o, inp_s inp)
{
    ...
    if (0 < dpad_y && inps_btn_jp(inp, INP_A)) {
        ...
        o->v_q12.x = dpad_x * HERO_VX_CRAWL;
    }
}
...
void hero_do_walking(g_s *g, obj_s *o, inp_s inp)
{
    ...
    bool32  can_start_crawl = !inps_btn(inp, INP_B) &&
                             ...
                             !o->rope;
    bool32 can_jump = 1;
    if (o->v_prev_q12.x == 0 && o->v_q12.x != 0) {
        o->animation = 0;
    }
    ...
    if (h->grabbing) {
        ...
        if (!can_sprint) {
            ...
            switch (dpad_x) {
            case -1: o->v_q12.x = min_i32(o->v_q12.x, -HERO_VX_WALK); break;
            case +1: o->v_q12.x = max_i32(o->v_q12.x, +HERO_VX_WALK); break;
            case +0: break;
            }
        }
        ...
        if (vs == 0 && h->skidding < 6) {
            ...
            if (0) {
                ...
                ax = min_i32(Q_VOBJ(0.08), HERO_VX_SPRINT - va);
            }
        ...
        if (HERO_VX_MAX_GROUND < abs_i32(o->v_q12.x)) {
            obj_vx_q8_mul(o, 252);
            if (abs_i32(o->v_q12.x) < HERO_VX_MAX_GROUND) {
                o->v_q12.x = sgn_i32(o->v_q12.x) * HERO_VX_MAX_GROUND;
            }
        }
        ...
        if (o->rope) {
            ...
            if (260 <= rope_stretch_q8(g, o->rope) &&
                sgn_i32(rn->p.x - o->pos.x) == -sgn_i32(o->v_q12.x)) {
                o->v_q12.x = 0;
            }
        } else if (o->v_q12.x == 0) {
            if (h->idle_state & 2) { // was idle prev frame
                h->idle_tick++;
                if (250 == h->idle_tick) {
                    h->idle_animID    = 1;
                    h->idle_anim_tick = 0;
    ...
    if (h->attack_tick) {
        o->v_q12.x = clamp_sym_i32(o->v_q12.x, HERO_VX_MAX_GROUND >> 2);
    }
}
                                      ...
                                      i32 *dt_snap_x)
{
    ...
    // x sensor positions ladder, width 16 == tile width
    {
        ...
        if (t1 && (t1->shape == TILE_LADDER ||
                   t1->shape == TILE_LADDER_ONE_WAY)) {
            // have to shift left
            *dt_snap_x = -(x1 & 15);
            return HERO_LADDER_VERTICAL;
        }
        if (t2 && (t2->shape == TILE_LADDER ||
                   t2->shape == TILE_LADDER_ONE_WAY)) {
            // have to shift right
            *dt_snap_x = 15 - (x2 & 15);
            return HERO_LADDER_VERTICAL;
        }
    ...
    // x sensor positions climbwall, width 8 == tile idth
    {
        ...
        tile_s *t2 = tile_map_at_pos(g, p2);
        if (t1 && t2 &&
            t1->shape == TILE_CLIMBWALL && t2->shape == TILE_CLIMBWALL) {
            *dt_snap_x = 0;
...
void hero_post_update(g_s *g, obj_s *o, inp_s inp)
{
    ...
    i32 vxa = abs_i32(o->v_q12.x);
    if (Q_VOBJ(3.0) <= vxa) {
        h->momentum = min_i32(h->momentum + 1, 50);
    } else if (vxa < Q_VOBJ(1.0)) {
        h->momentum = max_i32(h->momentum - 2, 0);
    } else if (vxa < Q_VOBJ(2.0)) {
        h->momentum = max_i32(h->momentum - 1, 0);
    }
    ...
    for (i32 n = 0; n < h->n_jumpstomped; n++) {
        ...
        { // jump or stomp
            ...
            case OBJID_MUSHROOM:
                ...
                o->v_q12.y = js.stomped ? -Q_VOBJ(10.0) : -Q_VOBJ(8.5);
                bounced    = 1;
                break;
            case OBJID_GEMPILE:
                gempile_on_hit(g, i);
    ...
    if (bounced) {
        o->v_q12.y = min_i32(o->v_q12.y, -Q_VOBJ(3.9));
    ...
    for (i32 n = 0; n < g->n_fluid_areas; n++) {
        ...
        if (overlap_rec(fr1, heroaabb)) {
            ...
            fluid_area_impact(fa, hcenter.x - fa->x, 12, imp >> 4, FLUID_AREA_IMPACT_COS);
        }
    }
    ...
    for (obj_each(g, it)) {
        ...
        if (it->flags & OBJ_FLAG_HURT_ON_TOUCH) {
            ...
            if (overlap_rec(heroaabb, it_rec_hurt)) {
        ...
        if (overlap_rec(heroaabb, obj_aabb(it))) {
            switch (it->ID) {
            case OBJID_HEARTPIECE:
                heartpiece_on_collect(g, it);
                break;
            case OBJID_LOOKAHEAD:
                lookahead_start(g, it);
                break;
            case OBJID_COIN: {
                coin_try_collect(g, it, hcenter);
                break;
            }
            case OBJID_HEALTHDROP: {
                o->health = min_i32(o->health + 2, o->health_max);
                obj_delete(g, it);
                break;
            }
    ...
    for (i32 y = by1; y <= by2; y++) {
        for (i32 x = bx1; x <= bx2; x++) {
            if (g->tiles[x + y * g->tiles_x].shape != TILE_SPIKES)
                continue;
    ...
    if (hero_dmg && !h->invincibility_ticks) {
        ...
        o->v_q12.x = hero_knockback.x << 4;
        o->v_q12.y = hero_knockback.y << 4;
    ...
    if (o->health && obj_grounded(g, o) && o->v_q12.x == 0) {
        coins_show_idle(g);
    }
