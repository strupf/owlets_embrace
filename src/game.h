...
#define map_obj_each(G, IT)                   \
    ...
    IT = (map_obj_s *)((byte *)IT + ((i32)IT->size_words << 2))
...
struct g_s {
    ...
    void           *map_objs;
    void           *map_objs_end;
    u32             hitbox_UID; // incremented per hitbox; always bigger than 0
    i32             n_hitboxes;
...
#define game_per_room_alloct(G, T)     (T *)game_per_room_alloc(G, sizeof(T), ALIGNOF(T))
#define game_per_room_alloctn(G, T, N) (T *)game_per_room_alloc(G, (N) * sizeof(T), ALIGNOF(T))
